...
export const getFiles = query({
    ...
    args: { projectId: v.id("projects") }, handler: async (ctx, args) => {
...
export const getFile = query({
    ...
    args: { id: v.id("files") }, handler: async (ctx, args) => {
...
export const getFolderContents = query({
    ...
    args: { projectId: v.id("projects"), parentId: v.optional(v.id("files")) }, handler: async (ctx, args) => {
...
export const renameFile = mutation({
    args: { id: v.id("files"), newName: v.string(), }, handler: async (ctx, args) => {
        ...
        const sibilings = await ctx.db.query("files").withIndex("by_project_parent", (q) => q.eq("projectId", file.projectId).eq("parentId", file.parentId)).collect();
        const existing = sibilings.find((sibiling) => sibiling.name === args.newName && sibiling.type === file.type && sibiling._id !== args.id);
...
export const updateFile = mutation({
    args: { id: v.id("files"), content: v.string(), }, handler: async (ctx, args) => {
        ...
        await ctx.db.patch("files", args.id, { content: args.content, updatedAt: now, });
