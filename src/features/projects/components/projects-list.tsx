...
const getProjectIcon = (project: Doc<"projects">) => {
    ...
    if (project.importStatus === "completed") { return <FaGithub className="size-3.5 text-muted-foreground" /> }
    if (project.importStatus === "failed") { return <AlertCircleIcon className="size-3.5 text-muted-foreground" /> }
    if (project.importStatus === "importing") { return <Loader2Icon className="size-3.5 text-muted-foreground animate-spin" /> }

    return <GlobeIcon className="size-3.5 text-muted-foreground" />
};

interface ProjectsListProps { onViewAll: () => void; }
...
export const ProjectsList = ({ onViewAll }: ProjectsListProps) => {
    ...
    // const projects = [mostRecent, ...rest];
            ...
            {rest.length > 0 && (
                <div className="flex flex-col gap-2">
                    ...
                    <ul className="flex flex-col">
                        {rest.map((project) => (<ProjectItem key={project._id} data={project} />))}
                    </ul>
                </div>
            )}
        </div>
