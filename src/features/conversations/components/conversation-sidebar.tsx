...
interface ConversationSidebarProps { projectId: Id<"projects">; };
...
export const ConversationSidebar = ({ projectId, }: ConversationSidebarProps) => {
    ...
    const handleSubmit = async (message: PromptInputMessage) => {
        ...
        if (isProcessing && !message.text) { setInput(""); return; }
        ...
        try {
            await ky.post("/api/messages", { json: { conversationId, message: message.text, }, });
        } catch (error) {
                ...
                <ConversationContent>
                    {conversationMessages?.map((message, messageIndex) => (
                        <Message key={message._id} from={message.role}>
                            <MessageContent>{message.status === "processing" ? (
